...
where the elements are arranged in a descending order
(the sorting happens as and when an element is enqueued)
and the element with the highest priority is the smallest element which is dequeued first*/
...
void enQueueByPriority(int element)
{
    ...
    // The logic of this program sorts the queue elements in descending order in below function call.
    else
...
void display()
{
    ...
    /*This for-loop not have a intial condition because for whatever is the value of front, check if its less than or equal to rear.
